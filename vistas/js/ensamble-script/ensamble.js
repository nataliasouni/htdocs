...
document.getElementById('AgregarEnsamble').addEventListener('click', function () {
  ...
  var datosTabla = [];
  document.querySelectorAll('#alertTable tbody tr').forEach(function (row) {
      var id = row.querySelector('td[name="Id"]').textContent;
      var cantidad = row.querySelector('.cantidad').value;
      var pendiente = row.querySelector('.pendiente').value;
      datosTabla.push({ Id: id, cantidad: cantidad, Pendiente: pendiente });
  });

  // Colocar los datos en el campo oculto como JSON
  document.getElementById('datosTabla').value = JSON.stringify(datosTabla);

  console.log("Datos antes de la conversión a JSON:", datosTabla);
